...
for (const char of charArray) {
  ...
  for (let i = 0; i < charsCoordinates.length; i++) {
    for (let j = i + 1; j < charsCoordinates.length; j++) {
      ...
      const antinodeA = [
        charsCoordinates[i][0] + distanceX,
        charsCoordinates[i][1] + distanceY,
      ];
      const antinodeB = [
        charsCoordinates[j][0] - distanceX,
        charsCoordinates[j][1] - distanceY,
      ];

      antinodeCoordinates.push(antinodeA);
      antinodeCoordinates.push(antinodeB);
    }
  }
}
